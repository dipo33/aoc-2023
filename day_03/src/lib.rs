...
pub fn test_first(result: u32) {
    assert_eq!(
        part1::execute("day_03/inputs/example1.txt", "", false),
        result,
...
pub fn test_second(result: u32) {
    assert_eq!(
        part2::execute("day_03/inputs/example2.txt", "", false),
        result,
...
pub fn measure(attempts: u32) -> (Duration, Duration) {
    ...
    for _ in 0..attempts {
        part1::execute("day_03/inputs/input.txt", "", false);
    }
    ...
    let beginning = Instant::now();
    for _ in 0..attempts {
        part2::execute("day_03/inputs/input.txt", "", false);
    }
